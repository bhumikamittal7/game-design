...
pygame.display.set_caption('Dance Recall Level 1')
...
while running:
        ...
        if event.type == pygame.QUIT:
          ...
          if not is_first_video_playing:
            ...
            if key in video_paths:
                ...
                if not iscorrectsequence and sequence != ['a', 'z', 'y', 'c', 'n', 'q'][:len(sequence)]:
                    issequenceincorrect = True
            else:
                sequence = []
                is_video_playing = False
                win = pygame.image.load("lost.png").convert()
                window.blit(win, (0, 0))
    ...
    if is_video_playing:
        ...
        if ret:
            ...
            if iscorrectsequence and key == 'q':
